...
const Review = () => {
  ...
  useEffect(() => {
    ...
    // DB에서 게시글을 받아오는 과정
    dbService.collection("board").doc('board').collection("review").orderBy("createdAt", "desc").onSnapshot((snapshot) => {
    ...
    return(           
        ...
        {/* 게시글을 보여주기 위한 middle 부분 */}
         ...
         {/* 페이지 개수에 맞게 페이지 번호를 만들어주고 클릭시에 그 페이지에 맞는 게시글을 보여줌 */}
             ...
             <div className={noti.register}>{IsManager ? <li><Link to="/Review_register">리뷰 등록하기</Link></li> :  <li><Link to="/Review_register">리뷰 등록하기</Link></li>}</div>    
            </div>             
          </div>
